...
import EmergencyAPIMonitor, { getEmergencyAPIHealthData } from '@/lib/emergency-monitoring';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = detailed ? healthStatus : {
...
async function performHealthChecks(): Promise<{
  ...
  try {
    ...
    const response = await fetch('http://localhost:3000/api/external/emergency?external=true', {
      method: 'HEAD',
      timeout: 5000,
    });
